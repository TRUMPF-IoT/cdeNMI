        ...
        if (tDataItem.length > 1) {
            ...
            if (!tFldContent) {
                ...
                if ((pFormField && cde.CBool(pFormField["ForceSet"])) || (tFldContent[tDataItem[tDataItem.length - 1]] !== pNewValue && (!pOldValues || pOldValues[tDataItem[tDataItem.length - 1]] !== pNewValue))) {
                    ...
                    //if ((pFormField.Flags & 1) != 0 && cde.MyContentEngine) 
                     //   tFldContent[tDataItem[tDataItem.length - 1]] = "&^CDEP5^&:" + cde.MyContentEngine.RSAEncrypt(pNewValue); //coming soon...RSA does fail at the moment
                    //else
